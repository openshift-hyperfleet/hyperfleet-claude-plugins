...
def safe_parse_iso8601:
  if . == null or . == "" then 0
  else (try parse_iso8601 catch 0) end;
...
def known_bots:
  ["coderabbitai", "coderabbitai[bot]", "openshift-ci[bot]", "openshift-ci",
    ...
    if length == 0 then {score: 0, detail: "No JIRA data fetched"}
      ...
      (if $ticket.sprint != null and $ticket.sprint.end_date != null then
        ...
        if $end_epoch <= 0 then 0
          ...
          if $bdays <= 0 then 3      # overdue
          elif $bdays <= 3 then 2    # ending in 1-3 business days
          elif $bdays <= 7 then 1    # ending this week
          else 0 end
        end
       else 0 end) as $boost |
...
# Provisional sort — LLM re-sorts with full confidence in Step 3c.4.
# Uses data_completeness as proxy for confidence (only component available pre-LLM).
sort_by([
  (-.deterministic_score),
  (-.data_completeness),
