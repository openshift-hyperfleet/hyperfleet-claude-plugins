...
assert_contains "JIRA link format" "<https://redhat.atlassian.net/browse/" "$SLACK_OUT"
...
# Tier 4 should NOT appear in Slack
if echo "$SLACK_OUT" | grep -q "Tier 4"; then
  printf "  ✗ Tier 4 should not appear in Slack output\n"
  FAIL=$((FAIL + 1))
else
  printf "  ✓ Tier 4 not shown in Slack output\n"
  PASS=$((PASS + 1))
fi
...
assert_contains "Tier table header" "| # | PR | JIRA | Status |" "$COMPACT_OUT"
