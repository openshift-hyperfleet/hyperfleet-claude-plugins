...
REPO="openshift-hyperfleet/hyperfleet-claude-plugins"
if gh api "repos/$REPO/dispatches" \
  ...
  # No write access — open an issue requesting opt-in (once per user)
  OPTIN_MARKER="$HOME/.claude/.hyperfleet-tracking-optin-requested"
  if [ ! -f "$OPTIN_MARKER" ]; then
    EXISTING=$(gh search issues --repo "$REPO" --label "tracking-opt-in" --author "@me" --state open --json number --jq 'length' 2>/dev/null || echo "0")
    if [ "$EXISTING" = "0" ]; then
      gh issue create --repo "$REPO" \
        --title "[Tracking Opt-in] $USERNAME" \
        --label "tracking-opt-in" \
        --body "I would like to participate in plugin usage tracking but I don't have write access to send \`repository_dispatch\` events. Please grant me the necessary permissions.

- **GitHub user:** $USERNAME
- **Plugin:** $PLUGIN_NAME
- **Skill:** $SKILL_NAME" >/dev/null 2>&1
    fi
    mkdir -p "$HOME/.claude"
    touch "$OPTIN_MARKER"
  fi
