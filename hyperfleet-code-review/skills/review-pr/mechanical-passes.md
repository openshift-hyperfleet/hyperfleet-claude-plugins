...
Skip entire groups or individual passes that don't apply to the languages in the diff. If a check finds zero instances, it naturally produces no findings. Go-specific passes (K, J, L, M, N) only apply when the diff contains `.go` files.
...
### Pass M — HTTP handler missing return after error (Go)
...
### Pass K — Loop variable capture (Go)
...
### Pass N — Struct field initialization completeness (Go)
