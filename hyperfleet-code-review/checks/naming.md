...
List every new or renamed identifier (variable, function, type, const, package) in the diff. Apply any project-specific naming conventions from the HyperFleet standards fetched in the Standards section in addition to the standard Go conventions below. If the standard was not fetched, apply only the Go conventions. For each, check:
